...
public class OverUnderDelimiter extends Atom {
	...
	public OverUnderDelimiter(Atom base, Atom script, SymbolAtom s, int kernUnit, float kern, boolean over)
	...
	public Box doCreateBox(TeXEnvironment env) {
		...
		Box del = DelimiterFactory.create(symbol, symbol.getName(), env, b.getWidth());
		...
		if (max - b.getWidth() > TeXFormula.PREC) {
			b = new HorizontalBox(this, b, max, TeXConstants.ALIGN_CENTER);
		}

		del = new VerticalBox(this, del, max, TeXConstants.ALIGN_CENTER);
		if (scriptBox != null && max - scriptBox.getWidth() > TeXFormula.PREC) {
			scriptBox = new HorizontalBox(this, scriptBox, max, TeXConstants.ALIGN_CENTER);
		}
