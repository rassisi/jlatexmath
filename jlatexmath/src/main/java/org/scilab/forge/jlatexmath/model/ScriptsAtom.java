...
public class ScriptsAtom extends Atom {
	...
	public Box doCreateBox(TeXEnvironment env) {
		Box b = (base == null ? new StrutBox(this, 0, 0, 0, 0) : base.createBox(env));
		Box deltaSymbol = new StrutBox(this, 0, 0, 0, 0);
		if (subscript == null && superscript == null)
			...
			HorizontalBox hor = new HorizontalBox(this, b);
			...
			if (base instanceof AccentedAtom) { // special case :
			...
			} else if (base instanceof SymbolAtom && base.type == TeXConstants.TYPE_BIG_OPERATOR) { // single big
																									// operator symbol
				...
				Box x = new CharBox(this, c);

				x.setShift(-(x.getHeight() + x.getDepth()) / 2 - env.getTeXFont().getAxisHeight(env.getStyle()));
				hor = new HorizontalBox(this, x);
				...
				if (delta > TeXFormula.PREC && subscript == null)
					hor.add(new StrutBox(this, delta, 0, 0, 0));
			...
			if (superscript == null) { // only subscript
				...
				HorizontalBox sup = new HorizontalBox(this, x, msiz, align);
				...
				if (style == TeXConstants.STYLE_DISPLAY)
					...
					p = tf.getSup2(style);
				shiftUp = Math.max(Math.max(shiftUp, p), x.getDepth() + Math.abs(tf.getXHeight(style, lastFontId)) / 4);
				...
				if (subscript == null) { // only superscript
					...
					HorizontalBox sub = new HorizontalBox(this, y, msiz, align);
					// add scriptspace (constant value!)
					...
					float interSpace = shiftUp - x.getDepth() + shiftDown - y.getHeight(); // space between sub- en
					// superscript
					if (interSpace < 4 * drt) { // too small
						...
						// above
						// baseline
						float psi = 4 * Math.abs(tf.getXHeight(style, lastFontId)) / 5 - (shiftUp - x.getDepth());
					...
					vBox.add(new StrutBox(this, 0, interSpace, 0, 0));
					vBox.add(sub);
