...
public class XLeftRightArrowFactory {
	...
	public static Box create(boolean left, TeXEnvironment env, float width) {
		...
		HorizontalBox hb = new HorizontalBox(minus.getAtom());
		float w;
	...
	public static Box create(TeXEnvironment env, float width) {
		...
		if (width < swidth) {
			HorizontalBox hb = new HorizontalBox(left.getAtom(), left);
			hb.add(new StrutBox(left.getAtom(), -Math.min(swidth - width, left.getWidth()), 0, 0, 0));
			hb.add(right);
			return hb;
		}
		...
		hb.add(new ScaleBox(minus.getAtom(), minus, (width - swidth - w) / minus.getWidth(), 1));
