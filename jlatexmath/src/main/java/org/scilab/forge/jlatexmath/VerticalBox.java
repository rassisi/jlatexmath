...
class VerticalBox extends Box {
	...
	public VerticalBox(Atom atom) {
		super(atom);
	}

	public VerticalBox(Atom atom, Box b, float rest, int alignment) {
		this(atom);
		add(b);
		if (alignment == TeXConstants.ALIGN_CENTER) {
			StrutBox s = new StrutBox(atom, 0, rest / 2, 0, 0);
			super.add(0, s);
			height += rest / 2;
			depth += rest / 2;
			super.add(s);
			...
			depth += rest;
			super.add(new StrutBox(atom, 0, rest, 0, 0));
		} else if (alignment == TeXConstants.ALIGN_BOTTOM) {
			height += rest;
			super.add(0, new StrutBox(atom, 0, rest, 0, 0));
		}
	}
	...
	public final void add(Box b, float interline) {
		if (children.size() >= 1) {
			add(new StrutBox(getAtom(), 0, interline, 0, 0));
		}
		add(b);
	}
