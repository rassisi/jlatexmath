...
public class AccentedAtom extends Atom {
	...
	/**
	 * Creates an AccentedAtom from a base atom and an accent symbol defined by its
	 * name
	 *
	 ...
	 *                                    ('acc')
	 * @throws SymbolNotFoundException    if there's no symbol defined with the
	 *                                    given name
	 */
	public AccentedAtom(Atom base, String accentName) throws InvalidSymbolTypeException, SymbolNotFoundException {
		...
		if (accent.type == TeXConstants.TYPE_ACCENT) {
			...
			throw new InvalidSymbolTypeException(
					"The symbol with the name '" + accentName + "' is not defined as an accent ("
							+ TeXSymbolParser.TYPE_ATTR + "='acc') in '" + TeXSymbolParser.RESOURCE_NAME + "'!");
	}

	/**
	 * Creates an AccentedAtom from a base atom and an accent symbol defined as a
	 * TeXFormula. This is used for parsing MathML.
	 *
	 * @param base base atom
	 * @param acc  TeXFormula representing an accent (SymbolAtom)
	 * @throws InvalidTeXFormulaException if the given TeXFormula does not represent
	 *                                    a single SymbolAtom (type
	 *                                    "TeXConstants.TYPE_ACCENT")
	 * @throws InvalidSymbolTypeException if the symbol is not defined as an accent
	 *                                    ('acc')
	 */
		...
		if (acc == null)
			...
			if (root instanceof SymbolAtom) {
				...
				if (accent.type == TeXConstants.TYPE_ACCENT)
					...
					throw new InvalidSymbolTypeException(
							"The accent TeXFormula represents a single symbol with the name '" + accent.getName()
									+ "', but this symbol is not defined as an accent (" + TeXSymbolParser.TYPE_ATTR
									+ "='acc') in '" + TeXSymbolParser.RESOURCE_NAME + "'!");
			} else
	...
	public Box doCreateBox(TeXEnvironment env) {
		...
		Box b = (base == null ? new StrutBox(this, 0, 0, 0, 0) : base.createBox(env.crampStyle()));
		...
		// create vertical box
		VerticalBox vBox = new VerticalBox(this);

		// accent
		Box y;
		float italic = ch.getItalic();
		Box cb = new CharBox(this, ch);
		if (acc)
			cb = accent.createBox(changeSize ? env.subStyle() : env);

		if (Math.abs(italic) > TeXFormula.PREC) {
			y = new HorizontalBox(this, new StrutBox(this, -italic, 0, 0, 0));
			y.add(cb);
		} else
			y = cb;
		...
		if (diff < 0)
			b = new HorizontalBox(this, b, y.getWidth(), TeXConstants.ALIGN_CENTER);
		vBox.add(y);

		// kern
		vBox.add(new StrutBox(this, 0, changeSize ? -delta : -b.getHeight(), 0, 0));
		// base
		vBox.add(b);
		...
		if (diff < 0) {
			HorizontalBox hb = new HorizontalBox(this, new StrutBox(this, diff, 0, 0, 0));
			hb.add(vBox);
			hb.setWidth(u);
			return hb;
		}
