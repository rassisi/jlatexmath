...
public class DefaultTeXFontParser {
	...
	/**
	 * if the register font cannot be found, we display an error message but we do
	 * it only once
	 */
	...
	private static class ExtensionParser implements CharChildParser {
		...
		public void parse(Element el, char ch, FontInfo info) throws ResourceParseException {
			...
			extensionChars[DefaultTeXFont.TOP] = DefaultTeXFontParser.getOptionalInt("top", el, DefaultTeXFont.NONE);
			extensionChars[DefaultTeXFont.MID] = DefaultTeXFontParser.getOptionalInt("mid", el, DefaultTeXFont.NONE);
			extensionChars[DefaultTeXFont.BOT] = DefaultTeXFontParser.getOptionalInt("bot", el, DefaultTeXFont.NONE);
	...
	public FontInfo[] parseFontDescriptions(FontInfo[] fi, InputStream file, String name)
		...
		FontInfo info = new FontInfo(Font_ID.indexOf(fontId), base, path, fontName, unicode, xHeight, space, quad, bold,
				roman, ss, tt, it);
	...
	public static Font createFont(InputStream fontIn, String name) throws ResourceParseException {
		try {
			...
			/**
			 * The following fails under java 1.5 graphicEnv.registerFont(f); dynamic load
			 * then
			 */
			...
			throw new XMLResourceParseException(
					RESOURCE_NAME + ": error reading font '" + name + "'. Error message: " + e.getMessage());
		} finally {
			try {
				if (fontIn != null)
					fontIn.close();
				...
				try {
					if (base == null) {
						map = factory.newDocumentBuilder()
								.parse(DefaultTeXFontParser.class.getResourceAsStream(include)).getDocumentElement();
					} else {
						map = factory.newDocumentBuilder().parse(base.getClass().getResourceAsStream(include))
								.getDocumentElement();
					}
				} catch (Exception e) {
	...
	public String[] parseDefaultTextStyleMappings() throws ResourceParseException {
		...
		Element defaultTextStyleMappings = (Element) root.getElementsByTagName("DefaultTextStyleMapping").item(0);
