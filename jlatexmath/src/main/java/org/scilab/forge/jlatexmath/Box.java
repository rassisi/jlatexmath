...
public abstract class Box {
	 ...
	 * be used. If it has no parent, the foreground color of the component on which
	 * it will be painted, will be used.
	 */
	protected Color foreground;

	/**
	 * The background color of the whole box. Child boxes can paint a background on
	 * top of this background. If it's null, no background will be painted.
	 */
	 ...
	 */
	protected Box(Atom atom) {
		this(atom, null, null);
	}

	/**
	 * Creates an empty box (no children) with all dimensions set to 0 and sets the
	...
	private javafx.geometry.Rectangle2D screenBox;
	...
	protected void drawDebug(Graphics2D g2, float x, float y) {
		...
		if (getAtom() != null && getAtom().getTexEnvironment() != null) {
			size = getAtom().getTexEnvironment().getSize();
		}
...
//		if (this instanceof CharBox) 
		{
			...
			screenBox = new javafx.geometry.Rectangle2D(x * 24, (y - height) * size, Math.abs(height) * 24,
					Math.abs(width) * 24);
	...
	public int getCaretPosition() {
		if (getAtom() != null) {
			return getAtom().getCaretPosition() + 1;
		}
		return -1;
	}

	public javafx.geometry.Rectangle2D getScreenBox() {
		return screenBox;
	}
