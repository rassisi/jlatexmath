...
public class BigOperatorAtom extends Atom {
	 ...
	 * @param under  atom representing the under limit
	 * @param over   atom representing the over limit
	 * @param limits whether limits should be drawn over and under the base (<-> as
	 *               scripts)
	 */
	...
	public Box doCreateBox(TeXEnvironment env) {
		...
		if ((limitsSet && !limits) || (!limitsSet && style >= TeXConstants.STYLE_TEXT)
			...
			if (base instanceof SymbolAtom && base.type == TeXConstants.TYPE_BIG_OPERATOR) { // single
				...
				delta = 0;
				y = new HorizontalBox(this, base == null ? new StrutBox(this, 0, 0, 0, 0) : base.createBox(env));
			}
			...
			// over
			if (over != null) {
				vBox.add(new StrutBox(this, 0, bigop5, 0, 0));
				x.setShift(delta / 2);
				vBox.add(x);
			...
			if (under != null) {
				...
				vBox.add(new StrutBox(this, 0, k, 0, 0));
				z.setShift(-delta / 2);
				vBox.add(z);
				vBox.add(new StrutBox(this, 0, bigop5, 0, 0));
			}
			...
			if (bbase != null) {
				HorizontalBox hb = new HorizontalBox(this, bbase.createBox(env));
				bbase.add(base);
				hb.add(vBox);
				base = Base;
				return hb;
	...
	private static Box changeWidth(Box b, float maxWidth) {
		if (b != null && Math.abs(maxWidth - b.getWidth()) > TeXFormula.PREC)
			return new HorizontalBox(b.getAtom(), b, maxWidth, TeXConstants.ALIGN_CENTER);
		else
			return b;
	}
}
