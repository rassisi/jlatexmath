...
public class DefaultTeXFont implements TeXFont {
	...
	static {
		...
		if (muFontId < 0 || muFontId >= fontInfo.length || fontInfo[muFontId] == null)
			throw new XMLResourceParseException(DefaultTeXFontParser.RESOURCE_NAME, DefaultTeXFontParser.GEN_SET_EL,
					DefaultTeXFontParser.MUFONTID_ATTR, "contains an unknown font id!");
	}
	...
	public static void addAlphabet(Character.UnicodeBlock alphabet, InputStream inlanguage, String language,
			InputStream insymbols, String symbols, InputStream inmappings, String mappings)
	...
	public static void addAlphabet(Character.UnicodeBlock alphabet, String name) {
		...
		try {
			DefaultTeXFont.addAlphabet(alphabet, TeXFormula.class.getResourceAsStream(lg), lg,
					TeXFormula.class.getResourceAsStream(sym), sym, TeXFormula.class.getResourceAsStream(map), map);
