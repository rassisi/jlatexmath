...
public class TeXParser {
	 ...
	 * @param isPartial   if true certains exceptions are not thrown
	 * @param parseString the string to be parsed
	 * @param firstpass   a boolean to indicate if the parser must replace the
	...
	/**
	 * Create a new TeXParser in the context of an array. When the parser meets a &
	 * a new atom is added in the current line and when a \\ is met, a new line is
	 * created.
	 *
	 * @param isPartial   if true certains exceptions are not thrown
	 * @param parseString the string to be parsed
	...
	/**
	 * Create a new TeXParser which ignores or not the white spaces, it's useful for
	 * mbox command
	 *
	 * @param isPartial   if true certains exceptions are not thrown
	 * @param parseString the string to be parsed
	 * @param firstpass   a boolean to indicate if the parser must replace the
	 *                    user-defined macros by their content
	 * @param space       a boolean to indicate if the parser must ignore or not the
	 *                    white space
	 * @throws ParseException if the string could not be parsed correctly
	 */
	...
	/**
	 * Return a boolean indicating if the character @ is considered as a letter or
	 * not
	 */
	 ...
	 * @param opts   must be 1 if the options are found before the first argument
	...
	/**
	 * Processes the given TeX command (by parsing following command arguments in
	 * the parse string).
	 */
	private Atom processCommands(String command) throws ParseException {
		...
		Atom atom = (Atom) mac.invoke(this, args);
	...
	/**
	 * Test the validity of a character in a command. It must contains only alpha
	 * characters and eventually a @ if makeAtletter activated
	 * 
	 * @param com the command's name
	 * @return the validity of the name
