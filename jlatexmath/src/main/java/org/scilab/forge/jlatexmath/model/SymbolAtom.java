...
public class SymbolAtom extends CharSymbol {
	...
	/**
	 * Constructs a new symbol. This used by "TeXSymbolParser" and the symbol types
	...
	public static SymbolAtom get(String name) throws SymbolNotFoundException {
		...
		if (obj == null) // not found
			...
			return (SymbolAtom) ((SymbolAtom) obj).clone();
	}
	...
	public Box doCreateBox(TeXEnvironment env) {
		...
		Box cb = new CharBox(this, c);
		if (env.getSmallCap() && unicode != 0 && Character.isLowerCase(unicode)) {
			try {

				CharBox cb1 = new CharBox(this,
						tf.getChar(TeXFormula.symbolTextMappings[Character.toUpperCase(unicode)], style));
				cb = new ScaleBox(this, cb1, 0.8, 0.8);
