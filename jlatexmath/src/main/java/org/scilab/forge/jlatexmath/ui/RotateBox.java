...
public class RotateBox extends Box {
	...
	public RotateBox(Atom atom, Box b, double angle, float x, float y) {
		super(atom);
		this.box = b;
		...
		xmax = (float) Math.max(-height * s,
				Math.max(depth * s, Math.max(width * c + depth * s, width * c - height * s))) + shiftX;
		xmin = (float) Math.min(-height * s,
				Math.min(depth * s, Math.min(width * c + depth * s, width * c - height * s))) + shiftX;
	...
	public RotateBox(Atom atom, Box b, double angle, Point2D.Float origin) {
		this(atom, b, angle, origin.x, origin.y);
	}

	public RotateBox(Atom atom, Box b, double angle, int option) {
		this(atom, b, angle, calculateShift(b, option));
	}
