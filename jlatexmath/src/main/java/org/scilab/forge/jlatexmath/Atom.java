	...
	public static int findCaretPosition(double x, double y) {
		...
		boxes.sort(new Comparator<Box>() {
			...
			public int compare(Box o1, Box o2) {
				try {
					Rectangle2D sc1 = o1.getScreenBox();
					Rectangle2D sc2 = o2.getScreenBox();
					...
					double fl1 = sc1.getWidth() * sc1.getHeight();
					double fl2 = sc2.getWidth() * sc2.getHeight();
					if (fl1 > fl2) {
						return 1;
					} else if (fl1 < fl2) {
						return -1;
...
//			if (b instanceof CharBox) 
			{
				if (b.getScreenBox() != null && b.getScreenBox().contains(x, y)) {

					System.out.println("Hit: " + b.getClass().getSimpleName() + "  at:  " + b.getScreenBox());

					if (b.getCaretPosition() != -1) {
						int newCaret = b.getCaretPosition();
						if (newCaret > caret) {
							caret = newCaret;
						}
					}
