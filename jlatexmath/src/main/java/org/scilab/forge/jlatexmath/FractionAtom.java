...
public class FractionAtom extends Atom {
	 ...
	 * @param num   the numerator
	 * @param den   the denominator
	 * @param noDef whether the default thickness should not be used for the
	 *              fraction line
	 * @param unit  a unit constant for the line thickness
	 * @param t     the thickness of the fraction line (in the given unit)
	 * @throws InvalidUnitException if the given integer is not a valid unit
	 *                              constant
	 */
	 ...
	 * @param num        the numerator
	 * @param den        the denominator
	 * @param defFactor  the thickness of the fraction line relative to the default
	 *                   thickness
	 * @param numAlign   alignment of the numerator
	 * @param denomAlign alignment of the denominator
	 */
	...
	/**
	 * The thickness of the fraction line is determined by the given value "t" in
	 * the given unit.
	 *
	 * @param num        the numerator
	 * @param den        the denominator
	...
	public Box doCreateBox(TeXEnvironment env) {
		...
		Box num = (numerator == null ? new StrutBox(this, 0, 0, 0, 0) : numerator.createBox(env.numStyle()));
		Box denom = (denominator == null ? new StrutBox(this, 0, 0, 0, 0) : denominator.createBox(env.denomStyle()));

		if (num.getWidth() < denom.getWidth())
			num = new HorizontalBox(this, num, denom.getWidth(), numAlign);
		else
			denom = new HorizontalBox(this, denom, num.getWidth(), denomAlign);
		...
		if (thickness > 0) { // WITH fraction rule
			...
			// fill vertical box
			vBox.add(new StrutBox(this, 0, kern1, 0, 0));
			vBox.add(new HorizontalRule(this, thickness, num.getWidth(), 0));
			vBox.add(new StrutBox(this, 0, kern2, 0, 0));
			...
			// fill vertical box
			vBox.add(new StrutBox(this, 0, kern, 0, 0));
		}
		...
		return new HorizontalBox(this, vBox, vBox.getWidth() + 2 * f, TeXConstants.ALIGN_CENTER);
	}
}
