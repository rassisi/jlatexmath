...
public class TStrokeAtom extends Atom {
	...
	public Box doCreateBox(TeXEnvironment env) {
		...
		CharBox T = new CharBox(this, env.getTeXFont().getChar(upper ? 'T' : 't', "mathnormal", env.getStyle()));
		CharBox B = new CharBox(this, ch);
		Box y;
		if (Math.abs(italic) > TeXFormula.PREC) {
			...
			y = B;
		Box b = new HorizontalBox(this, y, T.getWidth(), TeXConstants.ALIGN_CENTER);
		VerticalBox vb = new VerticalBox(this);
		vb.add(T);
		vb.add(new StrutBox(this, 0, -0.5f * T.getHeight(), 0, 0));
		vb.add(b);
		return vb;
	}
}
